...
public class CelebResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllCelebsByCommunityIsEqualToSomething() throws Exception {
        // Initialize the database
        Community community = CommunityResourceIntTest.createEntity(em);
        em.persist(community);
        em.flush();
        celeb.addCommunity(community);
        celebRepository.saveAndFlush(celeb);
        Long communityId = community.getId();

        // Get all the celebList where community equals to communityId
        defaultCelebShouldBeFound("communityId.equals=" + communityId);

        // Get all the celebList where community equals to communityId + 1
        defaultCelebShouldNotBeFound("communityId.equals=" + (communityId + 1));
    }


    @Test
    @Transactional
    public void getAllCelebsByProfileIsEqualToSomething() throws Exception {
        // Initialize the database
        Profile profile = ProfileResourceIntTest.createEntity(em);
        em.persist(profile);
        em.flush();
        celeb.addProfile(profile);
        celebRepository.saveAndFlush(celeb);
        Long profileId = profile.getId();

        // Get all the celebList where profile equals to profileId
        defaultCelebShouldBeFound("profileId.equals=" + profileId);

        // Get all the celebList where profile equals to profileId + 1
        defaultCelebShouldNotBeFound("profileId.equals=" + (profileId + 1));
    }
