...
public class InterestResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllInterestsByCommunityIsEqualToSomething() throws Exception {
        // Initialize the database
        Community community = CommunityResourceIntTest.createEntity(em);
        em.persist(community);
        em.flush();
        interest.addCommunity(community);
        interestRepository.saveAndFlush(interest);
        Long communityId = community.getId();

        // Get all the interestList where community equals to communityId
        defaultInterestShouldBeFound("communityId.equals=" + communityId);

        // Get all the interestList where community equals to communityId + 1
        defaultInterestShouldNotBeFound("communityId.equals=" + (communityId + 1));
    }


    @Test
    @Transactional
    public void getAllInterestsByProfileIsEqualToSomething() throws Exception {
        // Initialize the database
        Profile profile = ProfileResourceIntTest.createEntity(em);
        em.persist(profile);
        em.flush();
        interest.addProfile(profile);
        interestRepository.saveAndFlush(interest);
        Long profileId = profile.getId();

        // Get all the interestList where profile equals to profileId
        defaultInterestShouldBeFound("profileId.equals=" + profileId);

        // Get all the interestList where profile equals to profileId + 1
        defaultInterestShouldNotBeFound("profileId.equals=" + (profileId + 1));
    }
