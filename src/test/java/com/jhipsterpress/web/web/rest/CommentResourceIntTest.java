...
public class CommentResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllCommentsByProfileIsEqualToSomething() throws Exception {
        // Initialize the database
        Profile profile = ProfileResourceIntTest.createEntity(em);
        em.persist(profile);
        em.flush();
        ...
        // Get all the commentList where profile equals to profileId
        defaultCommentShouldBeFound("profileId.equals=" + profileId);

        // Get all the commentList where profile equals to profileId + 1
        defaultCommentShouldNotBeFound("profileId.equals=" + (profileId + 1));
    }
