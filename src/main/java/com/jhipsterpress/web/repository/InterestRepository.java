...
public interface InterestRepository extends JpaRepository<Interest, Long>, JpaSpecificationExecutor<Interest> {
    ...
    @Query(value = "select distinct interest from Interest interest left join fetch interest.communities left join fetch interest.profiles",
    ...
    @Query("select interest from Interest interest left join fetch interest.communities left join fetch interest.profiles where interest.id =:id")
