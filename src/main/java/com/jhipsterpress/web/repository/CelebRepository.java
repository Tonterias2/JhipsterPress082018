...
public interface CelebRepository extends JpaRepository<Celeb, Long>, JpaSpecificationExecutor<Celeb> {
    ...
    @Query(value = "select distinct celeb from Celeb celeb left join fetch celeb.communities left join fetch celeb.profiles",
    ...
    @Query("select celeb from Celeb celeb left join fetch celeb.communities left join fetch celeb.profiles where celeb.id =:id")
