...
public interface ActivityRepository extends JpaRepository<Activity, Long>, JpaSpecificationExecutor<Activity> {
    ...
    @Query(value = "select distinct activity from Activity activity left join fetch activity.communities left join fetch activity.profiles",
    ...
    @Query("select activity from Activity activity left join fetch activity.communities left join fetch activity.profiles where activity.id =:id")
